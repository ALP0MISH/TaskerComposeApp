...
import com.example.composetaskerapp.cammon.extensions.convertToColor
...
fun AddTaskScreen(
    ...
    Column(
        ...
        UserInputBlock(text = text, onValueChange = { value ->
            text = value
        ...
        AnimatedVisibility(
            ...
            TimePickerUI(
                ...
                height = 200.dp, textStyle = MaterialTheme.typography.titleLarge.copy(
                    fontWeight = FontWeight.Bold
                )
            )
        }
...
fun AddTaskToolbar(
    ...
    Row(
        modifier = modifier
            .padding(
                horizontal = LargeSpacing, vertical = ExtraMediumSpacing
            )
            .fillMaxWidth(),
    ) {
        Text(
        ...
        Column(
            ...
            Row(
                ...
                if (selectedTime != null) {
                    ...
                    Text(
                        text = selectedTime, style = MaterialTheme.typography.bodyMedium.copy(
                            color = if (isSystemInDarkTheme()) Color.White
                            else colorResource(id = R.color.large_gray)
                        )
                    )
                    ...
                    if (selectedDate != null) {
                        SpacerWidth(dp = MediumSpacing)
                        Text(
                            text = selectedDate, style = MaterialTheme.typography.bodyMedium.copy(
                                color = if (isSystemInDarkTheme()) Color.White
                                else colorResource(id = R.color.large_gray)
                            )
                        )
...
fun ParametersHeader(
    ...
    Column(
        ...
        Divider(
            color = if (isSystemInDarkTheme()) Color.White else colorResource(id = R.color.large_gray)
        )
        ...
        Row(
            ...
            Text(
                ...
                }, text = taskCategory.title, style = MaterialTheme.typography.bodyMedium.copy(
                    fontSize = 15.sp
                )
            )
            SpacerWidth(dp = MediumSpacing)
            CircleColorComponent(
                ...
                color = taskCategory.colorCode.convertToColor(),
            )
            onClick(ParametersHeaderType.TIME)
        }
    }
